...
public class GameOfLifeGrid implements CellGrid {
  ...
  public synchronized void next() {
    ...
    int col, row;
//    int neighbours;
    Enumeration<Cell> enumer;

    generations++;
    nextShape.clear();

    // Reset cells
    enumer = currentShape.keys();
    while ( enumer.hasMoreElements() ) {
      cell = (Cell) enumer.nextElement();
      cell.neighbour = 0;
    }
    // Add neighbours
  ...
  /**
   * Get Cell stored at a certain place
   * @param col x-coordinate of cell
   * @param row y-coordinate of cell
   * @return the Cell object, or null if it's not in the current shape
   */
  public synchronized Cell getCellObj( int col, int row ) {
  try {
      if ( currentShape.containsKey(grid[col][row] ))
      		{
    	  		return grid[row][col];
      		}
      	else 
